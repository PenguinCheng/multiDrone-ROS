...
private:
	...
	// the gps position, I add
	ros::Subscriber gps;
	// the height, I add
	ros::Subscriber alt;
	// the connect state, I add
	ros::Subscriber connect_state;
	...
	// a service to set the stream rate
	...
	void gpsSubCb(const sensor_msgs::NavSatFixConstPtr& msg);    //I add
	void altSubCb(const sensor_msgs::NavSatFixConstPtr& msg);
	void connectSubCb(const mavros_msgs::MavlinkPtr& fromPtr);
    ...
    // gps returned by mavros, I add
	sensor_msgs::NavSatFix last_gps;
	// height
	sensor_msgs::NavSatFix last_alt;
	//
	mavros_msgs::Mavlink last_connect_state;
...
public:
	...
	bool setStreamRate(unsigned char streamID, int rate);
