	...
	if(is_takeOff_finished ) {
		...
		while(nh.ok()) {
			...
			std_msgs::String gps_string;
      			std::stringstream ss;
      			ss << "x3o"<<selfGlobalPos.longitude<<"a"<<selfGlobalPos.latitude<<"#";
			...
			if(neigGlobalPos.lat == 0.0 || neigGlobalPos.lon == 0.0
				...
				if( (abs(x_offset) <= r1) && (abs(x_offset_) <= r1) ){
				...
				else if(abs(x_offset) <= r1) {
				    vel.twist.linear.x = -8*( (pow(x_offset,2)-2*pow(r1,2)+pow(r,2))*(pow(r1,2)-pow(r,2))*x_offset ) / pow((pow(x_offset,2)-pow(r,2)),3);
				}
				else if(abs(x_offset_) <= r1) {
				    vel.twist.linear.x = -8*( (pow(x_offset_,2)-2*pow(r1,2)+pow(r,2))*(pow(r1,2)-pow(r,2))*x_offset_ ) / pow((pow(x_offset_,2)-pow(r,2)),3);
				}
                		else {
  					vel.twist.linear.x = 0;
                		}

				if( (abs(y_offset) <= r1) && (abs(y_offset_) <= r1) ) {
				...
				else if(abs(y_offset) <= r1) {
				    vel.twist.linear.y = -8*( (pow(y_offset,2)-2*pow(r1,2)+pow(r,2))*(pow(r1,2)-pow(r,2))*y_offset ) / pow((pow(y_offset,2)-pow(r,2)),3);
				}
				else if(abs(y_offset_) <= r1) {
				    vel.twist.linear.y = -8*( (pow(y_offset_,2)-2*pow(r1,2)+pow(r,2))*(pow(r1,2)-pow(r,2))*y_offset_ ) / pow((pow(y_offset_,2)-pow(r,2)),3);
				}
                		else {
